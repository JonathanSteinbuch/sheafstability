...
inline void SparseScalarMatrix<Scalar>::add(unsigned int r, unsigned int c,const Scalar &value)
	{
		...
		auto ret = rows[r].insert(pair<unsigned int, Scalar>(c, value));
		if (!ret.second)
		{
...
template <typename Scalar>
	void SparseScalarMatrix<Scalar>::symmSwaps(vector<unsigned int>& newpos, bool invert)
	{
		vector<spRow<Scalar>> oldRows = rows;
		vector<spCol > oldCols = cols;

		vector<unsigned int> invnewpos(newpos.size());
		...
		rowPerm = Permutation(rowPerm.size(), invnewpos) * rowPerm;
		colPerm = Permutation(colPerm.size(), newpos) * colPerm;

	}

template <typename Scalar>
const Scalar SparseScalarMatrix<Scalar>::randomreadaccess(unsigned int r, unsigned int c) const
	{
...
SparseScalarMatrix<Scalar> SparseScalarMatrix<Scalar>::kernel( const program_options & opt)
	{
		...
		{
			spRow<Scalar> nrow;
			nrow.insert(pair<unsigned int, Scalar>(i, 1));
