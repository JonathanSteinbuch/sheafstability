...
template<typename _Scalar>
Poly<_Scalar>::Poly(PolyRing<_Scalar>* mB, const string& input) :
		Ring(mB)
{
...
//Make sure that the polynomial is minimally represented with respect to the monomial basis of the ring.
template<typename _Scalar>
...
//Divide out any common scalars of the coefficients.
...
//Reduce the polynomial with respect to the reducer polynomial, i.e. reducer = 0.
template<typename _Scalar>
bool Poly<_Scalar>::reduce(Poly<_Scalar>& reducer)
{
	...
	while (it != poly.rend())
	{
		...
		{ //if the monomial is divisible by the leading monomial of the reducer replace that monomial.
			...
			*this -= a * reducer;
			...
			//get the iterator back to the correct position
...
Poly<_Scalar>& Poly<_Scalar>::operator*=(const _Scalar &rhs)
{
	...
	{
		(*it).second *= rhs;
		if ((*it).second == 0) //make sure there are no zero monomials dangling around
			it = poly.erase(it);
		else
			it++;
	}
...
//Division. You should only do this if you know the right hand side polynomial divides this.
template<typename _Scalar>
Poly<_Scalar>& Poly<_Scalar>::operator/=(const _Scalar &rhs)
{
	assert(rhs != 0);
...
//Derivation including division by the factorial as occuring in the Taylor polynomial.
template<typename _Scalar>
...
//Return a polynomial consisting only of the i-th degree one monomial.
template<typename _Scalar>
...
//Return a somewhat generic polynomial of the given degree
template<typename _Scalar>
...
//return a submatrix describing multiplication by this polynomial in the given degrees.
//rowoffset and coloffset give the position of the submatrix in the bigger matrix.
//rows and cols give the dimensions of the submatrix
//The matrix entries will be returned as triplets (row, column, value).
//No checks are performed so be sure the given input is well-formed.
template<typename _Scalar>
...
//Homogenize by multiplying the monomials with powers of the given variable
template<typename _Scalar>
...
//Convenience function. For description see appendMatrix.
template<typename _Scalar>
...
//longform is commonly understood by most computer algebra systems
template<typename _Scalar>
